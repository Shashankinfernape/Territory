...
interface ShuffleProps {
  ...
  /** Skip ScrollTrigger and play after this many seconds (use in fixed overlays) */
  playOnMount?: boolean;
  playDelay?: number;
}
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      // ── Mount-mode: skip ScrollTrigger (for fixed overlays like splash) ──
      if (playOnMount) {
        const timer = gsap.delayedCall(playDelay, create);
        return () => {
          timer.kill();
          removeHover();
          teardown();
    ...
    },
    {
      dependencies: [
        ...
        shuffleDirection, shuffleTimes, animationMode, loop, loopDelay, stagger,
        scrambleCharset, colorFrom, colorTo, triggerOnce, respectReducedMotion,
        triggerOnHover, onShuffleComplete, playOnMount, playDelay,
      ],
      scope: ref,
    }
  );
